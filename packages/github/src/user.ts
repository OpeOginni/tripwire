...
export async function getRepoContributors(
	...
	try {
		...
		const BOT_LOGINS = new Set(["dependabot", "renovate", "github-actions", "codecov", "netlify", "vercel", "snyk-bot", "greenkeeper", "imgbot", "allcontributors"]);
		return result
...
export async function fetchUserContributions(
	...
	try {
		...
		const contributions: ContributionsData = {
			...
			weeks: (cal?.weeks ?? []).map((w: { contributionDays: Array<{ date: string; contributionCount: number }> }) => ({
		...
		const pinned: PinnedRepo[] = (u.pinnedItems?.nodes ?? [])
			...
			.map((n: Record<string, unknown>) => ({
				...
				primaryLanguage: n.primaryLanguage as { name: string; color: string | null } | null,
			}));
