...
export function RuleBuilderEditor({
	...
	const simulateRule = useMutation(
		trpc.customRules.simulate.mutationOptions(),
	);
	...
	return (
		<div className="flex h-full w-full">
			...
			<div className="flex-1 relative" ref={reactFlowWrapper}>
				...
				>
					<Background
						...
						color="#FFFFFF08"
					/>
					<Controls className="!bg-tw-card !border-tw-border !rounded-lg [&>button]:!bg-tw-card [&>button]:!border-tw-border [&>button]:!text-tw-text-muted [&>button:hover]:!bg-tw-hover" />
					{showMiniMap && (
						<MiniMap
							nodeColor={(n) => {
								if (n.id === outputNodeId) return "#34A6FF";
								return (
									nodeColors[n.type as keyof typeof nodeColors] ??
									"#9F9FA9"
								);
							}}
							maskColor="#0D0D0F99"
				...
				{outputNodeId && (
					<div className="absolute top-3 left-3 z-10 flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-tw-card/90 border border-tw-border text-[11px] text-tw-text-secondary">
						<span className="w-2 h-2 rounded-full bg-[#34A6FF]" />
			...
			<div className="w-[280px] shrink-0 border-l border-tw-border bg-tw-surface overflow-auto flex flex-col">
				<div className="px-3 pt-3 pb-2 flex flex-col gap-3">
					...
					<div className="flex flex-col gap-1">
						...
						<input
							...
							className="h-8 rounded-[10px] bg-tw-card px-2.5 text-[13px] text-white placeholder:text-[#6E6E6E] outline-none border border-transparent focus:border-tw-accent/40"
						/>
					</div>
						...
						<div className="bg-tw-card rounded-[10px] p-1 flex items-center gap-1">
							{(
								...
								<Button variant="ghost"
									...
									className={`flex-1 flex items-center justify-center h-7 px-1.5 rounded-[6px] text-[12px] font-medium transition-colors cursor-pointer ${
										action === val
					...
					{action === "threshold" && (
						<div className="flex flex-col gap-1">
							...
							<input
								...
								className="h-8 rounded-[10px] bg-tw-card px-2.5 text-[13px] text-white outline-none border border-transparent focus:border-tw-accent/40 w-20"
							/>
						</div>
					)}
					...
					<div className="flex flex-col gap-1.5">
						...
						<label className="flex items-center gap-2 text-[12px] text-tw-text-secondary cursor-pointer">
							<input
								type="checkbox"
								checked={scopePR}
				...
				<div className="border-t border-tw-border px-3 py-2.5 flex flex-col gap-2">
					<div className="text-[11px] uppercase tracking-[0.08em] text-tw-text-tertiary font-medium">
						Impact Preview
					</div>
					<Button variant="ghost"
						...
						<div className="flex flex-col gap-2 mt-1">
							<div className="grid grid-cols-3 gap-1.5">
								<div className="rounded-md bg-tw-card px-2 py-1.5 text-center">
									...
									<div className="text-[10px] text-tw-text-tertiary">
										Total
									</div>
								</div>
								<div className="rounded-md bg-tw-card px-2 py-1.5 text-center">
							...
							<div className="flex items-center gap-2">
								<div className="flex-1 h-2 rounded-full bg-tw-card overflow-hidden">
									<div
							...
							{simResult.blockPercentage > 50 && (
								<p className="text-[11px] text-tw-warning m-0">
									This rule would block more than half of recent
									contributors.
								</p>
							)}
							...
							{simResult.contributors.length > 0 && (
								<div className="flex flex-col gap-0.5 max-h-[200px] overflow-auto">
									{simResult.contributors.map((c) => (
										<div
											key={c.username}
											className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-tw-inner"
										>
											{c.avatarUrl && (
												<img
													src={c.avatarUrl}
											...
											<span className="text-[12px] text-tw-text-secondary truncate flex-1">
												{c.username}
											</span>
											<span
				...
				{saveError && (
					<div className="px-3">
						<p className="text-[11px] text-tw-error m-0">
							{saveError}
						</p>
					</div>
				)}
				...
				<div className="mt-auto border-t border-tw-border px-3 py-2.5">
					<Button variant="ghost"
						...
						className="w-full flex items-center justify-center gap-1.5 h-9 rounded-[10px] bg-tw-accent text-white hover:opacity-90 text-[13px] font-medium transition-colors disabled:opacity-50"
					>
						{isSaving
							? "Saving..."
							: isEditMode
