...
interface ChatComposerProps {
  ...
  /**
   * When set, text that starts with `/` is treated as a slash command.
   * The composer shows a command palette and routes execution through
   * this handler instead of `onSend`.
   */
  onSlashCommand?: (rawCommand: string) => Promise<{ ok: boolean }>
}
...
export function ChatComposer({
  ...
  async function sendMessage() {
    ...
    if (onSlashCommand) {
      ...
      if (parsed) {
        const result = await onSlashCommand(message)
        if (result.ok) {
          setText("")
          setMentions([])
          setDismissedTriggerKey(null)
  ...
  const slashEnabled = !!onSlashCommand
  const acceptPaletteSelection = async (cmd: SlashCommand) => {
    ...
    if (!parsed) return
    const result = await onSlashCommand(raw)
    if (result.ok) {
      setText("")
    }
  }
  ...
  const showSlashPalette = slashEnabled && slashInput.showPalette
  ...
  return (
    ...
    >
      ...
      {showSuggestions && !showSlashPalette ? (
        <div
          id={suggestionListId}
          role="listbox"
