...
import { PlayTriangleIcon13, UserCircleMutedIcon13 } from "#/components/icons/app-chrome-icons";
import {
	ReactFlow,
	Background,
	Controls,
			...
			<div className="flex-1 relative" ref={reactFlowWrapper}>
				...
				<div className="absolute top-3 right-3 flex items-center gap-1.5 z-10">
					<Button
						...
						className={showSim ? "bg-tw-card text-[#FAFAFA]" : "text-tw-text-muted hover:text-tw-text-primary"}
					>
						<PlayTriangleIcon13 />
						Test
					</Button>
...
function SimulationPanel({
	...
	const [userCard, setUserCard] = useState<{ login: string; avatarUrl: string; name: string | null } | null>(null);
	...
	const simInputs = useMemo(() => collectSimInputs(nodes, edges), [nodes, edges]);
	...
	const runSim = () => {
		setError(null);
		setSimStep(0);
		const results = simulateWorkflow(nodes, edges, mode, contextValues, actionLabels);
		setSimResults(results);
		setSimStep(0);
		setIsAnimating(true);
	};

	const clear = () => { setSimResults(null); setUserCard(null); setError(null); setSimStep(0); setIsAnimating(false); };
	...
	const groupedInputs = useMemo(() => {
		...
		for (const input of simInputs) {
			const group = input.source === "user" ? "User Data" : input.source === "content" ? "Content" : "Manual";
	...
	const hasUserInputs = Object.keys(groupedInputs).some((g) => g === "User Data");
	const hasContentInputs = Object.keys(groupedInputs).some((g) => g === "Content");

	return (
		<div className="w-[300px] shrink-0 border-l border-tw-border bg-tw-surface overflow-auto flex flex-col">
			<div className="px-3 pt-3 pb-2">
				<div className="flex items-center justify-between mb-1">
					<span className="text-[13px] font-medium text-tw-text-primary">Test run</span>
					{simResults && (
						<Button variant="ghost" size="xs" onClick={clear} className="text-[11px] text-[#FFFFFF40] hover:text-[#FFFFFF73]">Clear</Button>
					)}
				</div>
				<p className="text-[11px] text-[#FFFFFF40] leading-relaxed m-0 mb-3">
					Run your workflow against test data to see how each node evaluates.
				</p>

				<div className="bg-tw-card rounded-[10px] p-1 flex items-center gap-1 mb-3">
					{([
						...
						<Button variant="ghost"
							...
							className={`flex-1 flex items-center justify-center h-7 px-2 rounded-[6px] text-[12px] font-medium transition-colors cursor-pointer ${
								mode === m
			...
			{mode === "user" && (
				<div className="px-3 pb-3">
					<div className="flex items-center gap-2 h-9 rounded-[10px] bg-tw-card px-2.5 mb-2">
						...
						<Button
							...
							className="text-[11px] text-tw-accent shrink-0 h-auto p-0"
						>
						...
						<div className="flex flex-wrap gap-1 mb-2">
							{suggestions.map((s) => (
								<Button
									...
									className="gap-1.5 bg-tw-card hover:bg-tw-hover text-left"
								>
									<img src={`https://github.com/${s.username}.png?size=24`} alt="" className="size-4 rounded-full" />
									<span className="text-[11px] text-tw-text-secondary">{s.username}</span>
								</Button>
							))}
						</div>
					)}
					...
					{userCard && (
						<div className="flex items-center gap-2.5 mb-3">
							<img src={userCard.avatarUrl} alt="" className="size-7 rounded-full" />
							<div className="min-w-0 flex-1">
								<p className="text-[12px] font-medium text-tw-text-primary truncate m-0">{userCard.name ?? userCard.login}</p>
								<p className="text-[10px] text-[#FFFFFF40] m-0">@{userCard.login}</p>
							</div>
						</div>
					)}
					...
					{simInputs.length > 0 && (
						<>
							{hasUserInputs && (
								<div className="mb-3">
									<p className="text-[11px] text-[#FFFFFF40] m-0 mb-2">
										{userCard ? "Fetched from GitHub. Edit to test edge cases." : "Enter test values for the contributor profile."}
									</p>
									<div className="rounded-[10px] bg-tw-card overflow-hidden">
										{(groupedInputs["User Data"] ?? []).map((input, idx) => (
											<div
												key={input.key}
												className={`flex items-center justify-between gap-2 px-3 h-8 ${idx > 0 ? "border-t border-[#FFFFFF08]" : ""}`}
											>
												<span className="text-[11px] text-[#FFFFFF59] shrink-0">{input.label}</span>
												{input.type === "boolean" ? (
													<Button variant="ghost"
														type="button"
														onClick={() => setField(input.key, !contextValues[input.key])}
														className="text-[12px] text-tw-text-primary tabular-nums bg-transparent outline-none text-right"
													>
													...
													<input
														...
														onChange={(e) => setField(input.key, input.type === "number" ? Number(e.target.value) : e.target.value)}
														className="flex-1 min-w-0 bg-transparent text-[12px] text-tw-text-primary tabular-nums outline-none text-right"
														placeholder="0"
													/>
												)}
											</div>
							...
							{hasContentInputs && (
								<div className="mb-3">
									<p className="text-[11px] text-[#FFFFFF40] m-0 mb-2">Content to test against language and pattern rules.</p>
									{(groupedInputs["Content"] ?? []).map((input) => (
										<textarea
											...
											rows={3}
											className="w-full rounded-[10px] bg-tw-card px-3 py-2 text-[12px] text-tw-text-primary outline-none resize-none placeholder:text-[#6E6E6E]"
										/>
									))}
								</div>
							)}
			...
			{simResults && (
				<div className="flex-1 overflow-auto">
					<div className="flex items-center gap-3 px-3 py-2.5 border-t border-tw-border">
						<span className="text-[11px] text-tw-text-tertiary tabular-nums">{passCount} pass</span>
						<span className="text-[11px] text-tw-text-tertiary tabular-nums">{failCount} fail</span>
						<span className="text-[11px] text-tw-text-tertiary tabular-nums">{execCount} exec</span>
						{isAnimating && (
					...
					<div className="px-3 pb-3 flex flex-col gap-0.5">
						{visibleResults.map((r, i) => {
							...
							const statusText =
								r.status === "pass" ? "pass" :
								r.status === "fail" ? "fail" :
								r.status === "executed" ? "exec" :
								"skip";
							const statusColor =
								r.status === "pass" ? "text-[#FFFFFF59]" :
								r.status === "fail" ? "text-tw-error" :
								"text-[#FFFFFF40]";
							const isLatest = i === visibleResults.length - 1 && isAnimating;
							const isDelayWaiting = isLatest && r.pauseMs != null && r.pauseMs > 400;
							return (
								<div
									key={r.nodeId}
								...
								>
									<div className="flex items-center justify-between gap-2">
										<span className="text-[13px] text-tw-text-primary truncate">{label}</span>
										{isDelayWaiting ? (
											<div className="h-3 w-3 shrink-0 animate-spin rounded-full border-[1.5px] border-[#FFFFFF40] border-t-transparent" />
										) : (
											<span className={`text-[10px] tabular-nums ${statusColor} shrink-0`}>{statusText}</span>
										)}
									</div>
									{r.detail && (
										<p className="text-[11px] text-[#FFFFFF40] leading-relaxed mt-0.5">{r.detail}</p>
									)}
								</div>
							);
						})}
