...
export function EventPageExternalLinkIcon11({ className }: { className?: string }): React.ReactElement {
	return (
		<svg
			className={className}
			width="11"
...
export function EventIssueDotCircleIcon12({ color }: { color: string }): React.ReactElement {
	return (
	...
	return (
		<svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden>
			...
			<path
				...
				strokeLinecap="round"
			/>
			<path d="M18.5 8.5V14.5M21.5 11.5H15.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
		</svg>
	);
}

export function EventRuleResultGlyph({ result }: { result: string }): React.ReactElement {
	...
	if (result === "passed") {
		return (
			<svg width="16" height="16" viewBox="0 0 16 16" fill="#67E19F" aria-hidden>
