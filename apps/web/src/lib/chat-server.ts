...
export function mergeClientMessagesWithStored(
  ...
  for (const clientMessage of clientMessages) {
    const existing =
      typeof clientMessage.id === "string"
...
function applyApprovalResponses(storedMessage: any, clientMessage: any): void {
  if (
    !Array.isArray(storedMessage.parts) ||
    !Array.isArray(clientMessage.parts)
  )
    return
  ...
  for (const part of storedMessage.parts) {
    ...
    if (!id || part.state !== "approval-requested" || !part.approval?.id)
      continue
  ...
  for (const clientPart of clientMessage.parts) {
    ...
    storedPart.approval = {
      ...
      ...(clientPart.approval?.reason
        ? { reason: clientPart.approval.reason }
        : {}),
    }
  }
}
...
export function sanitizeMessages(
  ...
  for (let i = merged.length - 1; i >= 0; i--) {
    ...
    const hasOnlyResults =
      msg.parts.length > 0 &&
      msg.parts.every((p: any) => p.type === "tool-result")
  ...
  const result = merged
    .map((msg: any) => {
      if (msg.role === "tool") {
        if (!msg.tool_call_id || !completedCallIds.has(msg.tool_call_id))
          return null
        return msg
      }
  ...
  return result
    ...
    .filter(
      (msg: UIMessage | null): msg is UIMessage => !!msg && msg.parts.length > 0
    )
}
