...
import { workflows, repositories, organizations, events, workflowRuns } from "@tripwire/db";
...
export const workflowsRouter = {
	...
	runReport: authedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const { userData, contentText, contentMeta } = await fetchWorkflowRunContext({
				repoId: input.repoId,
			...
			const results = activeWorkflows.map((wf) => simulateWorkflowDefinition(wf, userData));
	...
	listInflightManualRuns: authedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			return { inflightWorkflowIds: [...new Set(rows.map((r) => r.workflowId))] };
		}),
	...
	manualRun: authedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const [row] = await db.select().from(workflows).where(eq(workflows.id, input.workflowId)).limit(1);
			...
			if (!workflowSupportsManualRun(row)) {
				throw new TRPCError({ code: "BAD_REQUEST", message: "Workflow has no manual trigger" });
			}

			let pullNum = input.pullNumber ?? -1;
			if (pullNum < -1) pullNum = -1;
			if (input.kind === "pr" && input.ref !== undefined && input.pullNumber === undefined) {
			...
			try {
				...
				if (isPostgresUniqueViolation(e)) {
					throw new TRPCError({
						code: "CONFLICT",
						message: "A run is already in progress for this workflow and scope.",
					});
				}
				throw e;
			}
			...
			try {
				...
				return { runId, status: "completed" as const, simulation: sim, userData, contentMeta, contentText };
			} catch (err) {
