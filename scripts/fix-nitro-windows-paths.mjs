...
try {
	...
	if (!nitroEntry) { console.log("No nitro found"); process.exit(0); }
...
c = c.split('from "${builtinHandler}";').join('from "${_fixWinPath(builtinHandler)}";');
// from "${plugin}"; → from "${_fixWinPath(plugin)}";
c = c.split('from "${plugin}";').join('from "${_fixWinPath(plugin)}";');

writeFileSync(filePath, c);
console.log("[fix-nitro-paths] Patched Nitro virtual templates for Windows paths.");
